...
### **核心创新点**
  ...
  * **Chronos 编码器迁移与特征融合：**
      ...
      * 通过池化层对原始多维时序数据进行降维后输入 Chronos 编码，提取出的时序特征与非时序特征在瓶颈层（Bottleneck Layer）进行融合，实现多模态信息的有效利用。

  * **改进型 SMOTE 时序数据增强：**

      * 针对 SMOTE 算法不适用于时序数据的缺陷，设计了三步走的改进流程：**时序平均 $\rightarrow$ 标准 SMOTE $\rightarrow$ 时序重构**。
      * **改进后的SMOTE方法示意图**  
